...
class Logement(db.Model):
    ...
    def availability_text(self):
        ...
        try:
            d = datetime.strptime(self.date_disponibilite, "%Y-%m-%d").date()
            if d <= date.today():
                return "Disponible maintenant"
            return f"Disponible à partir du {d.strftime('%d/%m/%Y')}"
        except ValueError:
            return f"Disponible à partir du {self.date_disponibilite}"

    class _MediaItem:
        """Lightweight wrapper so templates can call media.fichier / media.is_video."""
